...
from brain_core.experiments.lesions import (
    PathologyController,
    PathologyMutation,
    pathology_scenarios,
)
...
def test_ei_imbalance_and_delay_increase_change_dynamics_markers() -> Any:
    ...
    PathologyMutation(
        kind="ei_imbalance", scope="region", target="PFC", magnitude=0.3
    ).apply(state)
    PathologyMutation(
        kind="delay_increase", scope="edge", source="A", target="B", magnitude=0.4
    ).apply(state)
    ...
    cfg = validate_config(
        {
            ...
            "timestep": 0.005,
            "seed": 7,
            "task": {"scenario": "lesion-test", "duration": 5.0},
            "stimulus": {"scenario": "lesion-test", "source": "task"},
            "brain_profile": {"id": "default"},
            "clinical_profile": {
                "id": "hippocampal_lesion",
                "display_name": "Uszkodzenie hipokampa",
                "mechanism": "Testowa walidacja mutacji hipokampa.",
