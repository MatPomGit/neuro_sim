...
| ADR-0022 | proposed | Konfiguracje YAML i panele wyników Qt oparte na silniku | [`docs/adr/0022-konfiguracje-yaml-i-panele-qt-silnika.md`](adr/0022-konfiguracje-yaml-i-panele-qt-silnika.md) | — |
...
Dodatkowo rozszerzamy konfigurację `configs/multi_region_delay_*.yaml` oraz przykład `configs/snn_hippocampus_demo.yaml` o sekcję `snn`. Przykład jawnie zapisuje `neural_mass_regions`, region obwodu `HIP`, `sync_dt` oraz jednostki `Hz`/`fraction`, aby `SNNPopulationMapping` mógł być zweryfikowany statycznie przed uruchomieniem.
...
- raport analizy może pokazać różnicę między przebiegiem bazowym neural-mass i przebiegiem z lokalnym obwodem SNN bez zmiany surowych danych wejściowych.
