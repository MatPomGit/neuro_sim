...
| Timeline | `partial` | `event_timeline` w `brain_core/simulation/events.py` jest integrowany z silnikiem w `brain_core/simulation/engine.py` oraz raportami, a jego działanie weryfikują testy w `tests/test_task_protocols_and_engine.py`. Zdarzenia trialowe mają `trial_id` i numeryczne `trial_number`; metadane `details.trial_number` są spójne dla zdarzeń używanych w tabelach trial-by-trial. | Filtrowanie zdarzeń, pełniejsze linkowanie z wykresami i objaśnienia per profil kliniczny. |
